"""Run development-only V3 raw SEARCH/REPLACE protocol on consumed V1 selection tasks.

This is not a population-selection experiment. It reuses consumed V1 tasks only to
measure whether literal raw exact-replacement serialization removes V1 JSON/escaping
failures while leaving frozen models, resources, single-attempt generation, and the
protected evaluator unchanged. Any later selection claim requires a fresh untouched
selection pack after a new operational freeze.
"""
...
from .consumed_selection_development_v3 import (
...
from .local_operational_freeze_v1 import FINAL_CANDIDATE_IDS, FINAL_LOCAL_OPERATIONAL_CONFIG_FREEZE_V1
...
REPORT_SCHEMA = "plural-cognition-consumed-selection-development-v3"
OUTPUT_MANIFEST_SCHEMA = "plural-cognition-consumed-selection-development-output-manifest-v3"
        ...
        _canonical_json_bytes(
            {
                ...
                "development_protocol_sha256": DEVELOPMENT_PROTOCOL_SHA256_V3,
            }
        )
    ).hexdigest()
...
def _solver_prompt(blueprint: Any) -> bytes:
    ...
    if not raw.endswith(b"\n"):
        raise RuntimeError("development V3 prompt builder no longer ends in one LF")
    prompt = raw[:-1]
    if prompt.endswith(b"\n"):
        raise RuntimeError("development V3 prompt transport did not remove exactly one terminal LF")
    return prompt
...
def run_consumed_selection_development_v3(
    *,
    runtime_root: Path,
    model_root: Path,
    artifact_root: Path,
    ...
    (artifact_root / "development-protocol.json").write_bytes(_canonical_json_bytes(development_protocol_payload_v3()))
    ...
    for candidate_id in candidate_ids:
        ...
        for blueprint in blueprints:
            ...
            solved = False
            try:
                patch, parse_mode = extract_search_replace_patch(assistant, blueprint)
                validate_patch_against_blueprint(patch, blueprint)
            except ValueError as exc:
                parse_error = str(exc)
                invalid = {
                    "schema": "plural-cognition-consumed-selection-development-invalid-v3",
                ...
                if submission_sha256 != submission.sha256:
                    raise AssertionError("development V3 submission storage identity mismatch")
                ...
                if evaluation_sha256 != evaluation.sha256:
                    raise AssertionError("development V3 evaluation storage identity mismatch")
    ...
    if staging_root.exists() and any(staging_root.iterdir()):
        raise RuntimeError("consumed selection development V3 left Docker staging residue")
    report = {
    ...
    (artifact_root / "consumed-selection-development-v3.json").write_bytes(report_bytes)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run V3 raw SEARCH/REPLACE development on consumed V1 selection tasks.")
    ...
    try:
        report = run_consumed_selection_development_v3(
    ...
    print(f"output={args.artifact_root / 'consumed-selection-development-v3.json'}")
    return 0
