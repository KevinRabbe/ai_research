"""Run development-only V4 whole-file protocol on consumed V1 selection tasks.

This is not a population-selection experiment. It reuses consumed V1 tasks only to
measure whether whole-file candidate output removes anchor/serialization failures while
leaving frozen models, resources, single-attempt generation, and protected evaluation
unchanged. Any later selection claim requires a fresh untouched selection pack after a
new operational freeze.
"""
...
REPORT_SCHEMA_V4 = "plural-cognition-consumed-selection-development-v4"
OUTPUT_MANIFEST_SCHEMA_V4 = "plural-cognition-consumed-selection-development-output-manifest-v4"
        ...
        _canonical_json_bytes(
            {
                ...
                "development_protocol_sha256": DEVELOPMENT_PROTOCOL_SHA256_V4,
            }
        )
    ).hexdigest()
...
def _solver_prompt(blueprint: Any) -> bytes:
    raw = build_solver_prompt_v4(blueprint)
    if not raw.endswith(b"\n"):
        raise RuntimeError("development V4 prompt builder no longer ends in one LF")
    prompt = raw[:-1]
    if prompt.endswith(b"\n"):
        raise RuntimeError("development V4 prompt transport did not remove exactly one terminal LF")
    return prompt
...
def _write_output_manifest(root: Path) -> None:
    ...
    payload = {
        "schema": OUTPUT_MANIFEST_SCHEMA_V4,
        "development_protocol_sha256": DEVELOPMENT_PROTOCOL_SHA256_V4,
...
def run_consumed_selection_development_v4(
    *,
    runtime_root: Path,
    model_root: Path,
    artifact_root: Path,
    ...
    (artifact_root / "development-protocol.json").write_bytes(_canonical_json_bytes(development_protocol_payload_v4()))
    ...
    for candidate_id in candidate_ids:
        ...
        for blueprint in blueprints:
            ...
            solved = False
            try:
                patch, parse_mode = extract_full_file_patch_v4(assistant, blueprint)
                validate_patch_against_blueprint(patch, blueprint)
            except ValueError as exc:
                parse_error = str(exc)
                invalid = {
                    "schema": "plural-cognition-consumed-selection-development-invalid-v4",
                ...
                if submission_sha256 != submission.sha256:
                    raise AssertionError("development V4 submission storage identity mismatch")
                ...
                if evaluation_sha256 != evaluation.sha256:
                    raise AssertionError("development V4 evaluation storage identity mismatch")
    ...
    if staging_root.exists() and any(staging_root.iterdir()):
        raise RuntimeError("consumed selection development V4 left Docker staging residue")
    report = {
    ...
    (artifact_root / "consumed-selection-development-v4.json").write_bytes(report_bytes)
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run V4 whole-file development on consumed V1 selection tasks.")
    ...
    try:
        report = run_consumed_selection_development_v4(
    ...
    print(f"output={args.artifact_root / 'consumed-selection-development-v4.json'}")
    return 0
