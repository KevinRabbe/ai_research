...
def test_bootstrap_applies_strict_modify_create_delete_patch(tmp_path: Path) -> None:
    ...
    (repository / "obsolete.py").write_bytes(b"OLD = True\n")
    ...
    assert (repository / "module.py").read_bytes() == b"VALUE = 2\nREMOVE = True\n"
    assert (repository / "new.py").read_bytes() == b"CREATED = True\n"
...
def test_bootstrap_rejects_crlf_repository_source(tmp_path: Path) -> None:
    repository = tmp_path / "repository"
    repository.mkdir()
    (repository / "module.py").write_bytes(b"VALUE = 1\r\n")
    patch = (
    ...
    with pytest.raises(ValueError, match="patchable repository files must be UTF-8 LF text"):
        apply_unified_diff(repository, patch)
...
def test_bootstrap_rejects_rename_patch(tmp_path: Path) -> None:
    repository = tmp_path / "repository"
    repository.mkdir()
    (repository / "old.py").write_bytes(b"VALUE = 1\n")
    patch = (
        b"--- a/old.py\n"
        b"+++ b/new.py\n"
        b"@@ -1 +1 @@\n"
