...
from plural_cognition.recovery_cli import (
    ...
    T20M_TOKEN_BUDGET,
    resolve_t20m_screening_main,
    t20m_screening_intents,
)
...
def test_t20m_intents_change_training_budget_only_from_v12_capacity_matrix() -> None:
    ...
    assert all(intent.experiment_name == T20M_EXPERIMENT_NAME for intent in intents)
    assert all(intent.token_budget == T20M_TOKEN_BUDGET for intent in intents)
    assert all(intent.checkpoint_tokens == T20M_CHECKPOINT_TOKENS for intent in intents)
